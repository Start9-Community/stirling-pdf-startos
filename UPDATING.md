...
- Confirm the readiness endpoint the health check probes, `/api/v1/info/status`, still reports `UP`, and that the `SECURITY_*`, `SYSTEM_*`, `MANAGEMENT_*` and `MAIL_*` environment variables in `startos/main.ts` are still the names upstream reads.
